...
#' False negative test
#'
#' This function performs the false negative test. This test is designed to
#' determine the false negative rate of the Pcp and Mlp methods. To do this
...
#'
#'
#' @name falseNegativeTest
#' @rdname falseNegativeTest
#' @aliases falseNegativeTest
#' @author Jason Serviss
#' @keywords falseNegativeTest
...
#' @examples
#' falseNegativeTest(interval=50, iterations=1, cores=1, save=FALSE)
NULL
#' @export

falseNegativeTest <- function(
    interval = 1,
    iterations = 10000,
    cores = 4,
    save=TRUE,
    ...
    if(save == TRUE) {
        fnTestResults <- output
        save(
            fnTestResults,
            file=paste(outPath, "fnTestResults.rda", sep="/"),
            compress="bzip2"
        )
    }
    return(output)
